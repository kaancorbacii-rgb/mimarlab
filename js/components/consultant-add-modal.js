...
const ConsultantAddModal = (function () {
  ...
  function addSocialRow(platform, url) {
    ...
    row.innerHTML = `
      <select class="social-platform">${SOCIAL_PLATFORMS_LIST.map(p => `<option value="${p.value}"${p.value === platform ? ' selected' : ''}>${p.label}</option>`).join('')}</select>
      <input type="url" class="social-url" placeholder="https://..." value="${escapeAttr(url || '')}">
