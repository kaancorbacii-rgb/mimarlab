...
function createClaimCorrectionBox(config){
  ...
  async function loadClaimCard(){
    ...
    //
    // FİRMA/MARKA (kullanıcı isteği, 2026-09-10 dokuzuncu tur madde 2: "XL Mimarlık+Mühendislik
    // firmasına zaten bir yönetici atadım, 'Bu firma sana mı ait?' butonu hâlâ gözüküyor. Bir
    // firmaya veya markaya yönetici kullanıcı atayınca da bu buton kaybolsun."): kural artık
    // kişi/firma/marka için AYNI — onaylı bir sahip/yetkili varsa davet kutusu herkesten gizlenir.
    //
    // Bu, 2026-09-10 beşinci tur "birden fazla yönetici olabilsin" istisnasını (o tur bu kontrol
    // `profileType === 'architect'` ile sınırlandırılmıştı) BİLEREK geri alır. Çoklu yetkili hâlâ
    // mümkün — sunucu tarafı zaten destekliyor (profile_claims'in UNIQUE kısıtı (user_id,
    // profile_type, profile_key) ÜÇLÜSÜNDE) ve ikinci bir yetkili talebini bu kutu yerine Hesabım >
    // Profili Düzenle'deki Firma/Marka kutusundan (js/components/auth-modal.js, AYNI
    // profile_claims('office') talebini açar) ya da admin atamasıyla gönderebilir.
