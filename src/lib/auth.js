...
export async function getSessionUser(request, env) {
  ...
  const row = await env.DB.prepare(
    `SELECT u.id, u.email, u.username, u.name, u.dob, u.school, u.dept, u.photo_url, u.profession, u.position, u.awards, u.about, u.social_links, u.role, u.created_at
...
export function publicUser(user) {
  if (!user) return null;
  const { id, email, username, name, dob, school, dept, photo_url, profession, position, awards, about, social_links, role, created_at } = user;
  return {
    id, email, username: username || null, name, dob, school, dept, photoUrl: photo_url, profession, position,
