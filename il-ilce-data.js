// MİMARLAB — Türkiye'nin 81 ili ve ilçeleri. Proje Ekle formundaki Konum seçici (il -> ilçe basamaklı
// açılır pencere) ve parseLocation (proje.html filtreleri, proje-detay.html künyesi) için ortak
// referans veri — üç ayrı sayfada tekrarlanmayacak kadar büyük olduğu için (~970 ilçe) diğer küçük
// yardımcı fonksiyonlardan (ör. slugify) farklı olarak paylaşılan tek bir dosyada tutulur.
const IL_ILCE = {
  ...
  "Kahramanmaraş": ["Afşin","Andırın","Çağlayancerit","Dulkadiroğlu","Ekinözü","Elbistan","Göksun","Onikişubat","Nurhak","Pazarcık","Türkoğlu"],
...
const ILCE_TO_IL = {};
for(const il in IL_ILCE){ IL_ILCE[il].forEach(ilce => { if(!(ilce in ILCE_TO_IL)) ILCE_TO_IL[ilce] = il; }); }

// Künye/filtre metinlerinden {city, district} çıkarır — İstanbul'a özel eski davranışın yerini
// alır, artık 81 ilin tamamında ilçe -> il çözümlemesi yapar. "Fatih (Tarihi Yarımada)" gibi
// parantezli açıklamalar yalnızca il/ilçe olarak tanınmayan bir baş isim için ayrıştırılır.
function parseLocationFull(loc){
  if(!loc) return { city: null, district: null };
  const m = String(loc).match(/^([^(]+?)\s*(?:\(([^)]*)\))?$/);
  const head = (m ? m[1] : loc).trim();
  const paren = m && m[2] ? m[2].trim() : '';
  if(IL_LIST.includes(head)) return { city: head, district: paren || null };
