...
# Arşivde görünen canlı içerik (kullanıcı bildirimi, 2026-09-12) — bkz. src/lib/archiveSync.js.
if node scripts/test-2026-09-12-archive-live-guard.mjs >/tmp/preflight_arch 2>&1; then
  ok "arşiv canlı-içerik süzgeci testleri geçti ($(grep -c '^  ok ' /tmp/preflight_arch) test)"
else
  bad "arşiv canlı-içerik süzgeci testleri BAŞARISIZ:"
  tail -25 /tmp/preflight_arch >&2
fi
