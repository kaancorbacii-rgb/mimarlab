#!/usr/bin/env python3
"""64 Koleksiyon URL'lik üçüncü parti (kanepe/puf/masa sistemleri) kazıyıcı.

scrape-koleksiyon2.py'nin birebir eşi: `parse_koleksiyon` (RSC flight ayrıştırıcı)
scrape-batch67.py'den MODÜL OLARAK import edilir, gruplama koleksiyon3-groups.py'dedir.

Çıktı: scripts/output/koleksiyon3-raw.json — {url: parsed_dict}. Aile birleştirme BURADA
YAPILMAZ (koleksiyon3-build-payload.py'nin işi).

Kullanım: python3 scripts/scrape-koleksiyon3.py
"""
...
OUT = os.path.join(HERE, 'output', 'koleksiyon3-raw.json')
