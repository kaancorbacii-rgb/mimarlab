#!/usr/bin/env python3
"""Koleksiyon 4. parti (82 URL / 65 aile) içe aktarımı — import-koleksiyon3.py'nin eşi.
...
ölü placeholder'dan temizlenir — bkz. koleksiyon4-build-payload.py dosya başı (2).
...
bu partinin anahtarları `import/products/<slug>/k4-<n>.webp` — çakışmayan YENİ adresler.

Kullanım:
  python3 scripts/import-koleksiyon4.py --payload scripts/output/koleksiyon4-payload.json \
      [--dry-run] [--skip-images]
"""
import argparse
...
BATCH_TAG = 'k4'          # anahtar öneki — bkz. dosya başı "R2 ANAHTARI"
MAX_IMAGES = 14
DEAD_FILE_URL = 'pim.koleksiyon.com.tr/product-type-code/pdf'
TYPE_AXES = ('Tip', 'Fonksiyon')
_lock = threading.Lock()

# Koleksiyon URL segmenti -> insan okunur tip etiketi. koleksiyon4-build-payload.py'deki
...
SEGMENT_TYPE = {
    # --- önceki partilerden gelen (bu partide OLMAYAN) segmentler; tip ekseni geri doldurulurken
    #     eski versiyonlar için gerekli, bkz. yukarıdaki not ---
    ...
    # --- 4. parti (2026-09-05) segmentleri; koleksiyon4-build-payload.py#SEATING_TYPES ile
    #     BİREBİR aynı olmalı, aksi halde aynı sayfa iki farklı tip etiketi üretir ---
    'evde-ofis-masalari': 'Evde Ofis Masası', 'yemek-masalari': 'Yemek Masası',
    'sehpalar': 'Sehpa', 'kitaplik-ve-tv-uniteleri': 'Kitaplık & TV Ünitesi',
    'dolaplar-ve-bufeler': 'Dolap & Büfe', 'operasyonel-depolama': 'Keson / Pedestal',
    'yataklar': 'Yatak', 'komodinler': 'Komodin',
    'gece-sehpalari-ve-sifonyerler': 'Gece Sehpası & Şifonyer',
    'bolucu-panel': 'Bölücü Panel', 'masalar': 'Masa',
}
...
def main():
    ...
    out = os.path.join(HERE, 'output', 'koleksiyon4-import-report.json')
