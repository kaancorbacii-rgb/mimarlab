#!/usr/bin/env python3
"""koleksiyon3-raw.json (kazıma) + koleksiyon3-groups.py (gruplama) -> koleksiyon3-payload.json.

koleksiyon2-build-payload.py'nin İKİ EKSENLİ versiyon üretim desenini izler; bu partiye özgü
ÜÇ fark var:
...
1) EKSEN ADLARI ÜRÜN TİPİNE GÖRE DEĞİŞİR (görev metnindeki şartname):
   ...
   o ailelerin D1'de zaten çıplak kodlu versiyonları var (bkz. import-koleksiyon3.py'deki
...
2) ÖLÜ DOSYA BAĞLANTISI AYIKLANIR — GERÇEK BULGU (2026-09-05). Kazınan 370 modülün HEPSİNİN
   ...
   (D1'de bu bağlantıyı taşıyan 56 mevcut Koleksiyon satırı da import-koleksiyon3.py tarafından
   temizlenir.)
...
Kullanım:
  python3 scripts/koleksiyon3-build-payload.py
"""
...
_gspec = _ilu.spec_from_file_location('koleksiyon3_groups', os.path.join(HERE, 'koleksiyon3-groups.py'))
G = _ilu.module_from_spec(_gspec)
_gspec.loader.exec_module(G)

RAW = json.load(open(os.path.join(HERE, 'output', 'koleksiyon3-raw.json'), encoding='utf8'))
...
DESK_TYPES = {
    'masa-sistemleri': 'Operasyonel Çalışma İstasyonu',
    'yonetici-masalari': 'Yönetici Masası',
    'toplanti-masalari': 'Toplantı Masası',
    'seminer-masalari': 'Seminer Masası',
}
...
def build_family(group):
    ...
    for u, p in zip(urls, pages):
        ...
        if mods:
            ...
            # kaybolmasın (koleksiyon2-build-payload.py'deki AYNI gerekçe).
            variants.append({
    ...
    return {
        ...
        'files': all_files,
        'variants': variants,
        'batch_urls': urls,          # sayfa-bazlı overwrite için (import-koleksiyon3.py)
...
def shuffle_by_category(items):
    """koleksiyon2-build-payload.py#shuffle_by_type ile AYNI algoritma — INSERT sırası art arda
...
if __name__ == '__main__':
    items = shuffle_by_category([build_family(g) for g in G.ALL_GROUPS])
    slugs = set()
    for it in items:
    ...
    path = os.path.join(HERE, 'output', 'koleksiyon3-payload.json')
